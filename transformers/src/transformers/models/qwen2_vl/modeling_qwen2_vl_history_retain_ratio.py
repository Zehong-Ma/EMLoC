...
from .moba_efficient_video import moba_attn_varlen
...
class Qwen2VLSdpaAttention(Qwen2VLAttention):
    ...
    def forward(
        ...
        if custom_kv is not None:
            bs = query_states.shape[0]
            # if use_custom_group and custom_kv.get_seq_length()>256*8:
            #     chunk_num = 16
            #     chunk_len = custom_kv.get_seq_length()//chunk_num
            #     total_len = chunk_len*chunk_num
            #     grouped_k = torch.stack(list(custom_kv.key_cache[self.layer_idx][:, :, :total_len].expand(bs, -1, -1, -1).chunk(chunk_num, dim=-2))).permute(1,2,0,3,4)
            #     mean_k = grouped_k.mean(dim=-2)
            #     grouped_v = torch.stack(list(custom_kv.value_cache[self.layer_idx][:, :, :total_len].expand(bs, -1, -1, -1).chunk(chunk_num, dim=-2))).permute(1,2,0,3,4)
            #     mean_k = repeat_kv(mean_k, self.num_key_value_groups)
            #     group_attn = torch.matmul(query_states, mean_k.transpose(2, 3)) / math.sqrt(self.head_dim)
            #     top_s, top_ind = group_attn.reshape(bs, self.num_key_value_heads, self.num_key_value_groups, 
            #                                         q_len, chunk_num).sum(dim=(2,3)).topk(k=2, dim=-1)
                
            #     custom_k = torch.gather(grouped_k, dim=2, 
            #                  index=top_ind.unsqueeze(-1).unsqueeze(-1).expand(-1,-1,-1,chunk_len,self.head_dim)).contiguous().flatten(2,3)
            #     custom_v = torch.gather(grouped_v, dim=2, 
            #                  index=top_ind.unsqueeze(-1).unsqueeze(-1).expand(-1,-1,-1,chunk_len,self.head_dim)).contiguous().flatten(2,3)
            # else:
            custom_k = custom_kv.key_cache[self.layer_idx].expand(bs, -1, -1, -1)
            custom_v = custom_kv.value_cache[self.layer_idx].expand(bs, -1, -1, -1)
        ...
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        if output_attentions:
            if use_custom_group: # video group attention to accerate
                new_key_states = key_states[:,:,pre_len:-q_len]
                bs, n_k, new_seq_len, head_dim = new_key_states.shape
                block_size = 16
                if new_seq_len%block_size != 0:
                    max_chunk_len = (new_seq_len//block_size)*block_size
                    key_rep = new_key_states[:,:,:max_chunk_len].reshape(bs, n_k, new_seq_len//block_size, block_size, head_dim).mean(dim=-2)
                    key_rep = torch.cat([
                        key_rep,
                        new_key_states[:,:,max_chunk_len:].mean(dim=-2, keepdim=True)
                        ], dim=2) 
              
                else:
                    key_rep = new_key_states.reshape(bs, n_k, new_seq_len//block_size, block_size, head_dim).mean(dim=-2)
                attn_weights = torch.matmul(query_states, key_rep.transpose(2, 3)/math.sqrt(self.head_dim)).softmax(dim=-1).unflatten(1, (4,7)).mean(dim=2).unsqueeze(-1).repeat(1,1,1,1,block_size).flatten(3,4)[..., :new_seq_len]
            else:     
...
class Qwen2VLMoBAAttention(Qwen2VLAttention):
    """
    Qwen2 attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `Qwen2Attention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    ...
    def forward(
        ...
        if attention_mask is not None and q_len == key_states.shape[1]:
            ...
            attn_output_unpad = moba_attn_varlen(
                q=query_states,
                k=key_states,
                v=value_states,
...
QWEN2_VL_ATTENTION_CLASSES = {
    "eager": Qwen2VLMoBAAttention,
    "flash_attention_2": Qwen2VLFlashAttention2,
    "sdpa": Qwen2VLSdpaAttention,
    "moba": Qwen2VLMoBAAttention,
}
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def context_organize_bottom_up(
        ...
        all_token_num = 0
        delta_L_max =  0.005
        remain_nums =[int(input_ids.shape[-1]*r) for r in [1/10, 1/5, 1/2, 1]] #[ii*input_ids.shape[-1]//20 for ii in [1, 2, 5, 10]]
        # layer kv cache compression
        chunk_number = icl_input_ids.shape[0]
        chunk_iters = icl_input_ids.shape[0]//chunk_number + int((icl_input_ids.shape[0]%chunk_number)>0)
        
        icl_pre_hidden = icl_inputs_embeds
        next2_hidden = icl_inputs_embeds
    ...
    def context_organize(
        ...
        if icl_inputs_embeds is None:
            ...
            if icl_pixel_values is not None:
                ...
                icl_image_embeds = []
                for chunk_iter in range(chunk_iters):
                    pixel_length = icl_image_grid_thw[chunk_iter*chunk_number:(chunk_iter+1)*chunk_number].prod(dim=-1).sum()
                    sub_img_embeds = self.visual(icl_pixel_values[pixel_st:pixel_st+pixel_length], 
                                                 grid_thw=icl_image_grid_thw[chunk_iter*chunk_number:(chunk_iter+1)*chunk_number]
                                                 )
                    pixel_st += pixel_length
                    icl_image_embeds.append(sub_img_embeds)
                icl_image_embeds = torch.cat(icl_image_embeds, dim=0)
            ...
            if icl_pixel_values_videos is not None:
        ...
        ori_logits, ori_hidden_states, pooled_IQA_sigs =[], [], []
        for chunk_iter in range(chunk_iters):
            ...
            # icl_tmp_hidden_states= icl_output.hidden_states
            ori_logits.append(select_ori_logits)
            ori_hidden_states.append(icl_output.hidden_states)
            # process the attention scores, unmask the previous kv cache and important textual answer tokens
            label_mask = torch.nonzero(labels.squeeze()>-100).squeeze()
            icl_label_mask = torch.nonzero(icl_labels[st_:end_].squeeze()>-100).squeeze() #(labels>-100).squeeze(0) # do not need shift for loss
            head_dim = custom_kv.key_cache[0].shape[-1]
            kv_head_num = custom_kv.key_cache[0].shape[1]
            for ind, attn_ in enumerate(icl_output.attentions):
                attn_ = attn_[..., :-icl_labels.shape[-1]]
                bs, _, cur_seq_len, attn_k_num = attn_.shape
                # previous_chunk_len = attn_k_num-outputs.past_key_values.key_cache[ind].shape[-2]
                IQA_sig_scores = attn_.permute(0,2,1,3)[icl_label_mask[:,0], icl_label_mask[:,1]].sum(dim=0, keepdim=True)
                # group kv
                IQA_sig_scores = IQA_sig_scores.reshape(1, kv_head_num, -1, IQA_sig_scores.shape[-1]).sum(dim=-2)
                pooled_IQA_sig = F.avg_pool1d(IQA_sig_scores, kernel_size=5, padding=2, stride=1)
                ...
                # unmask the label tokens and previouly pruned tokens
                pooled_IQA_sig = pooled_IQA_sig.permute(2, 0, 1)
                # unmask_indices = torch.cat(
                #     [torch.arange(previous_chunk_len, dtype=label_mask.dtype).to(label_mask.device),
                #     previous_chunk_len + label_mask
                #     ],
                #     dim=0
                # )
                pooled_IQA_sig[label_mask] += 100.0
                pooled_IQA_sig = pooled_IQA_sig.permute(1, 2, 0) # bs, num_head, source_len
                if chunk_iter > 0:
                    pooled_IQA_sigs[ind] = pooled_IQA_sigs[ind] + pooled_IQA_sig
                else:
                    pooled_IQA_sigs.append(pooled_IQA_sig)
            del icl_output
        torch.cuda.empty_cache()
        ...
        all_token_num = 0
        delta_L_max = 0.008
        remain_nums = [ii*input_ids.shape[-1]//20 for ii in [2, 5, 10]]# [ii*input_ids.shape[-1]//32 for ii in [2, 4, 8, 16]] #[ii*input_ids.shape[-1]//20 for ii in [1, 2, 5, 10]]
        ...
        for layer_id in range(num_layers-1, -1, -groups):
            inner_kv = deepcopy(outer_kv)
            pre_len = outer_kv.pre_lens[layer_id] if len(outer_kv.pre_lens)>0 else 0
            remain_num = outer_kv.key_cache[layer_id].shape[-2]
            for cur_remain_num in remain_nums: #range(1, 5):
                # cur_remain_num = 433
                # cur_remain_num += (remain_num-input_ids.shape[-1]) 
                # cur_remain_num = decrease_order[layer_id]
                ...
                # inner_kv.key_cache[layer_id] = torch.cat([
                #     pre_k,
                #     torch.gather(cur_k, dim=2, index=remain_index).contiguous()
                # ], dim=2
                # )
                # inner_kv.value_cache[layer_id] = torch.cat([
                #     pre_v,
                #     torch.gather(cur_v, dim=2, index=remain_index).contiguous()
                # ], dim=2
                # )
                js_loss = 0
                for chunk_iter in range(chunk_iters):
                    ...
                    current_output = self.model(
                        input_ids=None,
                        start_layer_num=layer_id-group_id,
                        position_ids=icl_position_ids[:, st_:end_],
                        attention_mask=icl_attention_mask[st_:end_],
                        past_key_values=past_key_values,
                        inputs_embeds=ori_hidden_states[chunk_iter][layer_id-group_id],
                        use_cache=False,
                        output_attentions=output_attentions,
                        output_hidden_states=output_hidden_states,
                        return_dict=return_dict,
                    ...
                    eval_hidden_states = current_output[0][icl_labels[st_:end_]>-100]
                    select_new_logits = self.lm_head(eval_hidden_states).view(-1, self.config.vocab_size).to(torch.bfloat16)
                    # JS Divergence
                    stu_logits = F.softmax(select_new_logits, dim=-1, dtype=torch.bfloat16)
                    tea_logits = F.softmax(ori_logits[chunk_iter], dim=-1, dtype=torch.bfloat16)
                    M = (0.5*(stu_logits+tea_logits)).log()
                    js_loss_ = 0.5*(F.kl_div(M, torch.log(stu_logits), log_target=True, reduction="batchmean") + 
                                F.kl_div(M, torch.log(tea_logits), log_target=True, reduction="batchmean"))        
                    # M = (0.5*(F.softmax(select_new_logits, dim=-1)+F.softmax(ori_logits[chunk_iter], dim=-1))).log()
                    # js_loss_ = 0.5*(F.kl_div(M, F.log_softmax(select_new_logits,dim=-1), log_target=True, reduction="batchmean") + 
                    #             F.kl_div(M, F.log_softmax(ori_logits[chunk_iter],dim=-1), log_target=True, reduction="batchmean"))
                    js_loss += js_loss_
                    print("chunk_iter", js_loss_)
                    del current_output
                js_loss = js_loss/chunk_iters/groups  
                print("js loss:", js_loss)
                # KL divergence
                # kl_loss = F.kl_div(F.log_softmax(select_new_logits,dim=-1), select_ori_logits.softmax(dim=-1), reduction="batchmean")
                # print("kl_loss", kl_loss)
                if js_loss <= delta_L_max:
                    remain_num = cur_remain_num + pre_len
                    outer_kv.key_cache = inner_kv.key_cache
                    outer_kv.value_cache = inner_kv.value_cache
                    # outer_kv.value_cache[layer_id] = inner_kv.value_cache[layer_id]
                    break
            print(f"layer id {layer_id}: ", remain_num)
            all_token_num+=remain_num*groups
        torch.cuda.empty_cache()
        print("total_token_num: ", outer_kv.get_past_seq_len(), all_token_num/28)
        return outer_kv, custom_kv_pos_offset, all_token_num/28
    ...
    def compute_js_div(self, observe_tokens, select_ori_logits):
        select_new_logits = self.lm_head(observe_tokens).view(-1, self.config.vocab_size).to(torch.bfloat16)
        stu_logits = F.softmax(select_new_logits, dim=-1, dtype=torch.bfloat16)
        tea_logits = F.softmax(select_ori_logits, dim=-1, dtype=torch.bfloat16)
        M = (0.5*(stu_logits+tea_logits)).log()
        js_loss = 0.5*(F.kl_div(M, torch.log(stu_logits), log_target=True, reduction="batchmean") + 
    ...
    def video_context_organize(
        ...
        all_token_num = 0
        delta_L_max = 0.05
        remain_ratios = [ii/100 for ii in [10]]
        groups = 7
        pre_remain_ratio = []
        
        outer_kv = custom_kv
       
        select_ori_logits = self.lm_head(icl_hidden_states[:,-16:]).view(-1, self.config.vocab_size).to(torch.bfloat16)
        
        
        if self.history_remain_ratios is None:
            self.history_remain_ratios = torch.tensor([0.05]*num_layers, dtype=torch.float32).to(select_ori_logits.device)
        
        # utilize historyh_remain_ratios
        inner_kv = self.clone_kv_cache(outer_kv)
        for layer_id in range(num_layers): # range(num_layers): #
            remain_num = outer_kv.key_cache[layer_id].shape[-2]
            pre_compressed_len = outer_kv.key_cache[layer_id].shape[-2] - input_ids.shape[-1]
            cur_remain_ratio = self.history_remain_ratios[layer_id]
            ...
            inner_kv.key_cache[layer_id] = torch.cat([
                outer_kv.key_cache[layer_id][:,:,:pre_len],
                torch.gather(outer_kv.key_cache[layer_id][:,:,pre_len:], dim=2, index=remain_index).contiguous()
            ], dim=2
            )
            inner_kv.value_cache[layer_id] = torch.cat([
                outer_kv.value_cache[layer_id][:,:,:pre_len],
                torch.gather(outer_kv.value_cache[layer_id][:,:,pre_len:], dim=2, index=remain_index).contiguous()
            ], dim=2
            )
            all_token_num += cur_remain_num + pre_compressed_len
        ...
        current_output = self.model(
            ...
            attention_mask=icl_attention_mask,
            past_key_values=past_key_values,
            inputs_embeds=icl_tmp_hidden_states[0],
            use_cache=False,
            output_attentions=False,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ...
        if js_loss <= delta_L_max:
            ...
            for layer_id in range(num_layers-1, -1, -groups): # range(num_layers): #
                ...
                for cur_remain_ratio in remain_ratios: #range(1, 5):
                    ...
                    cur_remain_num = int(cur_remain_ratio*input_ids.shape[-1])
                    # cur_remain_num += pre_compressed_len
                    # cur_remain_num = decrease_order[layer_id]
                    # create index
                    for group_id in range(groups):
                        remain_index = pooled_IQA_sigs[layer_id-group_id].topk(cur_remain_num, dim=-1)[1]
                ...
                self.history_remain_ratios[layer_id-groups+1:layer_id+1] = (0.1*remain_ratio*torch.ones_like(self.history_remain_ratios[layer_id-groups+1:layer_id+1]) +
                                                                            0.9*self.history_remain_ratios[layer_id-groups+1:layer_id+1])
               
        return outer_kv, custom_kv_pos_offset, all_token_num/28, self.history_remain_ratios
