...
class DynamicCache(Cache):
    ...
    def __init__(self, num_hidden_layers: Optional[int] = None) -> None:
        ...
        self.pre_lens: List[torch.Tensor] = []
        self.pre_ratios = None
        
        # self.label_sts: List[torch.Tensor] = []
    ...
    def merge_layer_kv(self, key_state, value_state, layer_id):
        if len(self.pre_lens)<=layer_id:
            self.pre_lens.append(self.key_cache[layer_id].shape[-2])
        else:
            self.pre_lens[layer_id] = self.key_cache[layer_id].shape[-2]
        self.key_cache[layer_id] = torch.cat([self.key_cache[layer_id], key_state], dim=-2)
        self.value_cache[layer_id] = torch.cat([self.value_cache[layer_id], value_state], dim=-2)
        return 
    
    def merge_other_kv(self, other_kv, is_online_video=False):
        self._seen_tokens += other_kv._seen_tokens
        self.pre_lens = []
        for layer_idx in range(len(self.key_cache)):
            if is_online_video is False:
                self.pre_lens.append(self.key_cache[layer_idx].shape[-2])
            self.key_cache[layer_idx] = torch.cat([self.key_cache[layer_idx], other_kv.key_cache[layer_idx]], dim=-2)
            self.value_cache[layer_idx] = torch.cat([self.value_cache[layer_idx], other_kv.value_cache[layer_idx]], dim=-2)
        return
    
    def set_pre_ratios(self, pre_ratios):
        self.pre_ratios = pre_ratios
